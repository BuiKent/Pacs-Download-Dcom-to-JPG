...
Test ở đây thao túng trực tiếp _ConcurrencyGate (đơn vị nhỏ nhất, nhanh,
không cần FFmpeg thật) — tách biệt với test_video_engine.py vốn test hành vi
nghiệp vụ (trim/concat/...) bằng FFmpeg thật. Phần cuối file có 1 test tích
hợp gọi FFmpeg thật qua transcode() để xác nhận gate thật sự được áp dụng
trên đường đi thật, không chỉ trên _ConcurrencyGate cô lập.
"""
...
class TestConcurrencyGateUnit:
    ...
    def test_allows_up_to_limit_simultaneously(self):
        ...
        def worker(idx):
            with gate:
                entered.append(idx)
                barrier.wait()  # cả 2 phải vào được cùng lúc mới qua được barrier
                time.sleep(0.05)
    ...
    def test_third_task_waits_until_slot_frees(self):
        ...
        t1.start()
        time.sleep(0.02)  # đảm bảo t1 chắc chắn vào trước
        t2.start()
        t1.join(timeout=5)
        t2.join(timeout=5)

        # task 1 phải kết thúc TRƯỚC khi task 2 bắt đầu — chứng minh không
        # chạy song song khi limit=1
    ...
    def test_raises_server_busy_when_wait_timeout_exceeded(self):
        ...
        holder.start()
        time.sleep(0.05)  # đảm bảo holder đã chiếm slot
    ...
    def test_stats_reflects_running_and_waiting_counts(self):
        ...
        w.start()
        time.sleep(0.05)  # cho waiter kịp vào trạng thái chờ
    ...
    def test_exception_inside_with_block_still_releases_slot(self):
        """Nếu tác vụ bên trong `with gate:` ném lỗi, slot vẫn phải được trả
        lại — nếu không, gate sẽ bị 'rò rỉ' và các tác vụ sau treo vĩnh viễn."""
        ...
        # nếu slot không được trả lại, dòng dưới sẽ treo/ServerBusyError
        with gate:
            pass  # phải vào được ngay, không lỗi


class TestModuleLevelGates:
    """Test các gate module-level thật (_heavy_gate/_light_gate) mà mọi hàm
    nghiệp vụ (trim/concat/transcode/probe...) đi qua — xác nhận
    configure_concurrency() và concurrency_stats() hoạt động đúng trên
    chính các gate mà production code dùng, không phải bản sao cô lập."""

    def teardown_method(self):
        # Khôi phục giới hạn mặc định sau mỗi test để không rò rỉ cấu hình
        # sang các test khác chạy sau (thứ tự test không nên ảnh hưởng nhau).
    ...
    def test_heavy_and_light_gates_are_independent(self):
        """Bão hoà heavy gate không được chặn light gate — probe/thumbnail
        (light) vẫn phải chạy được ngay cả khi mọi slot heavy đang bận
        transcode/concat, vì đó chính là lý do tách 2 hạng mục."""
        ...
        # light gate phải còn trống hoàn toàn, không bị heavy gate ảnh hưởng
...
class TestTranscodeUsesGateForReal:
    """Test tích hợp: gọi transcode() thật (FFmpeg thật) 2 lần đồng thời với
    heavy_limit=1, xác nhận lần gọi thứ 2 phải chờ chứ không chạy song song —
    chứng minh gate thật sự nằm trên đường đi production, không chỉ đúng khi
    test cô lập _ConcurrencyGate."""
    ...
    def test_second_transcode_waits_for_first_when_limit_is_one(self, sample_video, tmp_path):
        ...
        t1.start()
        time.sleep(0.03)  # đảm bảo t1 chắc chắn giành slot trước
        t2.start()
        t1.join(timeout=60)
        t2.join(timeout=60)
        ...
        # Với heavy_limit=1, task 2 không thể "done" trước task 1 "done" vì
        # cả 2 đều phải giữ trọn slot suốt quá trình encode — nếu chạy song
        # song thật (bug), thứ tự hoàn thành có thể đảo ngược tự do theo tốc
        # độ máy; ở đây ta xác nhận task 1 luôn xong trước vì được cấp slot
        # trước và giữ độc quyền tới khi xong.
    ...
    def test_probe_still_works_while_heavy_gate_saturated(self, sample_video, tmp_path):
        """Xác nhận đúng lợi ích thực tế của việc tách gate: bác sĩ A đang
        export video (heavy) không chặn bác sĩ B chỉ đang xem thumbnail/mở
        hồ sơ khác (light)."""
        ve.configure_concurrency(heavy_limit=1, heavy_wait_timeout_s=30)
        release_event = threading.Event()
        ...
        # probe() dùng light gate — phải chạy được ngay dù heavy đang bị giữ
        start = time.time()
