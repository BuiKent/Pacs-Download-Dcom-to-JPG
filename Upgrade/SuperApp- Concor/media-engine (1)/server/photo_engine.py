...
# ---------------------------------------------------------------------------
# Giới hạn đồng thời — cùng lý do như video_engine.py: nhiều request cùng mở
# ảnh scan lớn (A4 300dpi ~2480x3508, có thể tới 8000x8000 theo _MAX_DIMENSION)
# đồng thời sẽ cộng dồn RAM nhanh (mỗi ảnh RGB 8000x8000 ~180MB chỉ để decode).
# Pillow không phải subprocess riêng như FFmpeg nên không cần semaphore giữa
# các tiến trình hệ điều hành, nhưng vẫn cần giới hạn số ảnh lớn được xử lý
# cùng lúc trong tiến trình Python để tránh tràn bộ nhớ khi nhiều bác sĩ cùng
# thao tác. Dùng chung cơ chế _ConcurrencyGate với video_engine để nhất quán.
# ---------------------------------------------------------------------------


class ServerBusyError(Exception):
    """Hàng đợi xử lý ảnh đã đầy quá lâu — khác về bản chất với lỗi ảnh/
    tham số (PhotoEngineError)."""
...
# Pillow xử lý ảnh đơn lẻ nhanh hơn nhiều so với encode video, nên giới hạn
# rộng hơn video_engine — chặn chủ yếu để tránh tràn RAM với ảnh cực lớn,
# không phải để tránh nghẽn CPU như FFmpeg.
...
def _open_safely(path: str | Path) -> Image.Image:
    ...
    with _heavy_gate:
        try:
            ...
            # QUAN TRỌNG: kiểm tra kích thước NGAY SAU open(), TRƯỚC khi gọi
            # load(). Image.open() chỉ đọc header (rẻ, mili-giây); load() mới
            # là bước giải nén toàn bộ pixel vào RAM (tốn CPU/RAM thật). Một
            # PNG nén tốt (ảnh 1 màu) có thể chỉ vài trăm KB trên đĩa nhưng
            # giải nén ra hàng trăm MB RAM nếu kích thước pixel lớn — đây là
            # kiểu tấn công "decompression bomb" kinh điển. Pillow có sẵn
            # Image.MAX_IMAGE_PIXELS nhưng mặc định chỉ WARN chứ không chặn
            # (đã xác nhận bằng thực nghiệm: ảnh 10000x10000 vẫn load() thành
            # công sau ~3.7s dù vượt ngưỡng cảnh báo). Vì vậy phải tự chặn
            # bằng kích thước từ header trước khi cho phép giải nén.
            ...
            img.load()  # ép decode ngay để bắt file hỏng sớm, không chỉ lazy-load lỗi sau
        except PhotoEngineError:
            raise
        except Exception as exc:  # Pillow ném nhiều loại lỗi khác nhau tuỳ định dạng hỏng
            raise PhotoEngineError(f"File ảnh hỏng hoặc không đọc được: {exc}") from exc

    # EXIF orientation: ảnh chụp từ điện thoại/máy ảnh thường có cờ xoay trong
    # EXIF thay vì xoay pixel thật — áp dụng ngay để mọi thao tác sau (crop,
    # toạ độ redact) tính đúng trên ảnh "nhìn thấy", không phải ảnh raw lệch.
...
# ---------------------------------------------------------------------------
# Thumbnail — cho thumbnail rail (mockup .thumb)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Crop / rotate — thao tác hình học cơ bản
# ---------------------------------------------------------------------------
...
def rotate(src: str | Path, out_path: str | Path, degrees: int, quality: int = 92) -> Path:
    """degrees dương = theo chiều kim đồng hồ, khớp icon 'Xoay 90°' trong mockup."""
    if degrees % 90 != 0:
        raise PhotoEngineError("Chỉ hỗ trợ xoay theo bội số 90° (chất lượng không mất mát).")
    img = _open_safely(src)
    rotated = img.rotate(-degrees, expand=True)  # PIL rotate dương = ngược kim đồng hồ, nên đảo dấu
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
# ---------------------------------------------------------------------------
# Redaction — che thông tin định danh. Ghi đè pixel thật (không phải overlay
# hiển thị), đúng yêu cầu dữ liệu y tế: bản đã "Lưu bản đã sửa" phải thật sự
# không còn khôi phục được nội dung bị che.
# ---------------------------------------------------------------------------
...
def redact_all_faces_heuristic(src: str | Path) -> list[Rect]:
    """Chỗ trống có chủ đích cho phát hiện khuôn mặt/text tự động sau này
    (vd. face_recognition, hoặc OCR + regex số CMND/số điện thoại qua
    pytesseract). Không triển khai ở bản này để tránh phụ thuộc nặng
    (dlib/opencv) khi bạn chưa xác nhận cần — nhưng chữ ký hàm để sẵn cho
    'Che tất cả định danh' trong mockup gọi vào khi bạn quyết định làm."""
    raise NotImplementedError(
        "Phát hiện tự động chưa triển khai. Cần chọn cách nhận diện "
        "(OCR + regex, hoặc model nhận diện khuôn mặt) trước khi code."
    )


# ---------------------------------------------------------------------------
# Annotate — chèn chữ / mũi tên / khung chỉ lên ảnh (burn-in vĩnh viễn)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Xuất PDF nhiều trang — khớp nút "Xuất PDF" trong mockup viewer ảnh/bệnh án
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Undo/redo — lịch sử thao tác dạng "công thức" (không lưu N bản ảnh đầy đủ),
# tránh phình bộ nhớ/đĩa khi người dùng sửa nhiều bước trên ảnh scan lớn.
# ---------------------------------------------------------------------------
...
class EditSession:
    """Giữ danh sách thao tác + áp dụng lại từ ảnh gốc mỗi lần render/export.
    Rẻ về bộ nhớ (chỉ lưu tham số, không lưu bitmap trung gian) và đúng với
    yêu cầu "bản gốc không bị ghi đè" — ảnh gốc luôn chỉ đọc trong suốt phiên.
    """
