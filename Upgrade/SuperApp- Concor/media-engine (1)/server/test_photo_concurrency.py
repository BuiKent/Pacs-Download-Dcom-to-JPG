...
class TestPhotoConcurrencyGate:
    ...
    def test_third_open_waits_when_limit_is_one(self, tmp_path):
        ...
        # Không thể chèn sleep vào bên trong _open_safely mà không sửa production
        # code, nên xác nhận gián tiếp: chạy N lần đồng thời với limit=1 và log
        # thời điểm — nếu gate hoạt động, tổng thời gian sẽ >= tổng thời gian đơn
        # lẻ x N (tuần tự hoá), thay vì ~bằng thời gian 1 lần (chạy song song).
        n = 6
        ...
        # Không assert thời gian tuyệt đối (máy chậm/nhanh khác nhau), chỉ xác
        # nhận không có lỗi/treo và toàn bộ N lệnh gọi đều hoàn thành.
    ...
    def test_raises_server_busy_when_saturated_and_timeout_short(self, tmp_path):
        ...
        img_path = tmp_path / "sample.jpg"
        Image.new("RGB", (4000, 4000), "white").save(img_path)  # ảnh lớn để giữ slot đủ lâu
    ...
    def test_exception_during_open_still_releases_slot(self, tmp_path):
        ...
        # nếu slot không được trả lại, lệnh gọi hợp lệ sau đây sẽ bị treo/lỗi
