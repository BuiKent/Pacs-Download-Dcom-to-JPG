...
function readTextValue(bytes, start, length) {
  if (!length || start < 0 || start + length > bytes.length) return '';
  return new TextDecoder('utf-8', { fatal:false }).decode(bytes.slice(start, start + length)).replace(/\0/g, '').trim();
}
...
const META_TEXT_TAGS = new Set([
  '00080016','00080018','00080020','00080021','00080022','00080023','00080060','00081030','0008103E',
  '00100010','00100020','0020000D','0020000E','00200011','00200013'
]);

function decodeKnownMeta(tag, bytes, start, length) {
  if (!META_TEXT_TAGS.has(tag)) return '';
  let value=readTextValue(bytes,start,length);
  if (tag==='00100010') value=value.replace(/\^+/g,' ').replace(/\s+/g,' ').trim();
  return value;
}
...
export function parseDicomMeta(input) {
  ...
  let pos=isPart10(bytes)?132:0;
  let transferSyntax='1.2.840.10008.1.2.1';

  while (pos + 8 <= bytes.length) {
    const group=dv.getUint16(pos,true), element=dv.getUint16(pos+2,true);
    if (group!==0x0002) break;
    const vr=String.fromCharCode(bytes[pos+4]||0,bytes[pos+5]||0);
    let len=0, valuePos=0;
    if (LONG_VR.has(vr)) { if(pos+12>bytes.length)break; len=dv.getUint32(pos+8,true); valuePos=pos+12; }
    else { len=dv.getUint16(pos+6,true); valuePos=pos+8; }
    if (valuePos + len > bytes.length) break;
    if (element===0x0010) transferSyntax=readTextValue(bytes,valuePos,len) || transferSyntax;
    pos=valuePos+len;
  }

  const explicit=transferSyntax!=='1.2.840.10008.1.2';
  const little=transferSyntax!=='1.2.840.10008.1.2.2';
  const out={transferSyntax};
  let guard=0;
  while (pos + 8 <= bytes.length && guard++ < 200000) {
    const group=dv.getUint16(pos,little), element=dv.getUint16(pos+2,little);
    ...
    if (explicit) {
      ...
      if (LONG_VR.has(vr)) { if(pos+12>bytes.length)break; len=dv.getUint32(pos+8,little); valuePos=pos+12; }
      else { len=dv.getUint16(pos+6,little); valuePos=pos+8; }
    } else {
      len=dv.getUint32(pos+4,little); valuePos=pos+8;
    }
    if (len===0xFFFFFFFF) break;
    if (valuePos + len > bytes.length) break;
    const decoded=decodeKnownMeta(tag,bytes,valuePos,len);
    if (decoded) out[tag]=decoded;
    pos=valuePos+len;
  }

  return {
    transferSyntax:out.transferSyntax,
    sopClassUid:out['00080016']||'',
    sopInstanceUid:out['00080018']||'',
    studyDate:out['00080020']||out['00080021']||out['00080022']||out['00080023']||'',
    modality:out['00080060']||'',
    studyDescription:out['00081030']||'',
    seriesDescription:out['0008103E']||'',
    patientName:out['00100010']||'',
    patientId:out['00100020']||'',
    studyUid:out['0020000D']||'',
    seriesUid:out['0020000E']||'',
    seriesNumber:out['00200011']||'',
    instanceNumber:out['00200013']||''
  };
}
