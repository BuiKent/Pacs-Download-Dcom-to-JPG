...
async function waitForDetector(tabId,timeoutMs=20000){
  ...
  while(Date.now()-start<timeoutMs){
    ...
    setWaiting(true,'Đang bắt manifest…',`${last?.viewerShell?'Đã thấy viewer/wrapper. ':''}Đã ghi nhận ${n} endpoint PACS.`);
...
async function analyze(){
  ...
  clearStudyUI();setWaiting(true,'Đang phân tích…','Kiểm tra wrapper, iframe và network của study hiện tại.');
  try{
    let s=await scan({silent:true});
    if(detectorReady(s)){
      try{await performAnalyze();return;}catch(e){/* reload fallback below */}
    }
    // Manifest may have been emitted before the panel opened. Reset only capture data,
    // then force exactly one reload so webRequest sees wrapper + all iframe traffic.
    await send('RESET_CAPTURE',{tabId:tab.id});
    setWaiting(true,'Đang tải lại viewer một lần…','Extension sẽ tự bắt network từ lúc wrapper/iframe khởi tạo.');
    await chrome.tabs.reload(tab.id,{bypassCache:true});
    s=await waitForDetector(tab.id,20000);
    if(!detectorReady(s)){
      const frame=(s?.frameUrls||[]).find(u=>u&&u!==s.currentUrl)||s?.bestViewerUrl||'';
      throw new Error(frame
        ? `Đã thấy viewer thật (${frame}) nhưng chưa thấy manifest/DICOM endpoint sau khi reload.`
        : 'Không tìm thấy iframe viewer hoặc manifest/DICOM endpoint sau khi reload.');
    }
