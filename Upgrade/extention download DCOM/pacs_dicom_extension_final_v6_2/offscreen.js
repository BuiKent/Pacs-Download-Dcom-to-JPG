...
async function writeViaDownloads(studyFolder,relativePath,bytes,job){const blob=new Blob([bytes],{type:'application/dicom'}),url=URL.createObjectURL(blob);let id=null;try{id=await chrome.downloads.download({url,filename:`PACS_DICOM/${studyFolder}/${relativePath}`,conflictAction:'overwrite',saveAs:false});job.activeDownloadIds.add(id);await waitDownload(id);}finally{if(id!=null)job.activeDownloadIds.delete(id);URL.revokeObjectURL(url);}}
...
chrome.runtime.onMessage.addListener((m,_s,sendResponse)=>{
  ...
  if(m.type==='CANCEL_ENGINE'){const j=jobs.get(Number(m.tabId));if(j){j.cancelled=true;j.controller.abort();for(const id of(j.activeDownloadIds||[]))chrome.downloads.cancel(id).catch(()=>{});emit(j,true);}sendResponse({ok:true});return false;}
