...
async function fetchRaw(url,task,accept='',signal){
  return await fetchStreamWithTimeout(url,task,accept,signal,globalSemaphore,headers);
}
...
async function probeDicomPrefix(task){
  async function attempt(useRange){
    await globalSemaphore.acquire();try{
    ...
    }catch{return false;}finally{globalSemaphore.release();}
  }
