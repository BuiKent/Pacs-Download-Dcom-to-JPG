/**
 * Sequence độ dài không xác định không được làm parser bỏ cuộc.
 *
 * Máy Hitachi (PACS VietMy) ghi (0008,1140) ReferencedImageSequence dạng
 * undefined-length ngay trước nhóm 0010, nên nếu parser dừng ở đó thì mất sạch
 * PatientName/PatientID/StudyInstanceUID và thư mục lưu thành "Unknown - NoID".
 */
...
/** SQ với độ dài 0xFFFFFFFF, đóng bằng Sequence Delimitation Item. */
...
/** Item độ dài không xác định, đóng bằng Item Delimitation Item. */
...
// SQ lồng nhau + item không xác định độ dài: ca khó nhất mà parser phải qua được.
...
if (meta.patientName !== 'NGUYEN THI VAN') throw new Error(`PatientName sau SQ bị mất: "${meta.patientName}"`);
if (meta.patientId !== 'BG20260730-188') throw new Error(`PatientID sau SQ bị mất: "${meta.patientId}"`);
if (meta.studyUid !== '1.2.392.200036.9123.1.1') throw new Error(`StudyUID sau SQ bị mất: "${meta.studyUid}"`);
if (meta.studyDate !== '20260730') throw new Error('StudyDate trước SQ phải còn nguyên');
if (!meta.hasPixelData) throw new Error('Phải tìm thấy Pixel Data');
if (!validatePart10(dcm).ok) throw new Error('validatePart10 phải chấp nhận file này');
