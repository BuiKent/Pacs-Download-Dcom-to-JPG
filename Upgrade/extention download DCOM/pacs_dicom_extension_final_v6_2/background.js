...
async function startTracking(tabId,manual=false){const s=await getTabState(tabId);s.tracking='watching';if(manual)s.manual=true;await saveTabState(tabId,s);await injectContent(tabId);await setBadge(tabId);scheduleAnalyze(tabId,250);if(manual)setTimeout(()=>maybeRecaptureVietmy(tabId),450);return s;}
...
async function materializeLearnedManifest(tabId,url,requestMeta){const s=await getTabState(tabId);let payload;try{payload=await fetchJsonFor(s,url,'application/json, application/dicom+json, text/json, */*',requestMeta);}catch{return{valid:[],discovered:0};}const discovered=extractUrlsFromJson(payload,url);for(const u of discovered){const h=classifyPacsUrl(u);if(h)await rememberRequest(tabId,u,{method:'GET',source:'learned-manifest'});}const likely=discovered.filter(u=>/\.dcm(?:\?|$)/i.test(u)||/(?:wado|dicom|instance|image|getimage|study-get-public)/i.test(u)).slice(0,48);if(!likely.length)return{valid:[],discovered:discovered.length};await ensureOffscreen();const probes=likely.map(u=>({url:u,headers:headersForUrl(s,u),method:'GET'}));const valid=[];for(let i=0;i<probes.length;i+=12){const r=await chrome.runtime.sendMessage({target:'offscreen',type:'PROBE_DICOM_URLS',probes:probes.slice(i,i+12)}).catch(()=>null);if(Array.isArray(r?.valid))valid.push(...r.valid);}if(valid.length){const fresh=await getTabState(tabId);for(const u of valid){await learnUrl(u,'dicom');pushUnique(fresh.genericDirectUrls,cleanUrl(u),5000);fresh.genericDirectMeta[cleanUrl(u)]={contentType:'application/octet-stream',learned:true};}fresh.confidence=Math.max(Number(fresh.confidence)||0,96);await saveTabState(tabId,fresh);scheduleAnalyze(tabId,100);}return{valid,discovered:discovered.length};}
