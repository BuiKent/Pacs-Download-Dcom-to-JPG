...
export const DicomwebAdapter={
  ...
  match(summary, state){
    ...
    if(Boolean(bestDetectedRequest(summary?.requests||[],['QIDO_SERIES','QIDO_INSTANCES','DICOM_METADATA','DICOM_INSTANCE','DICOM_FRAME','WADO','MEDDREAM_STRUCTURE'])))return true;
  ...
  async analyze(ctx){
    ...
    const meddream=bestDetectedRequest(ctx.summary.requests,['MEDDREAM_STRUCTURE']);
    let seed=qido?.url||meddream?.url||bestDetectedRequest(ctx.summary.requests,['QIDO_INSTANCES','DICOM_METADATA','DICOM_INSTANCE','DICOM_FRAME','WADO'])?.url;
    if(!seed){
    ...
    if(!raw){
      try{
        ...
        else raw=await ctx.fetchJson(seriesUrl,'application/dicom+json, application/json');
      }catch(err){
    ...
    if(!series.length){
      ...
      if(!series.length){
        const medSeries = Array.isArray(raw?.study?.series) ? raw.study.series : (Array.isArray(raw?.series) ? raw.series : []);
        if(medSeries.length){
          for(let i=0;i<medSeries.length;i++){
            const ms=medSeries[i],su=String(ms.uid||ms.SeriesInstanceUID||'').trim();
            if(!su)continue;
            const count=Array.isArray(ms.instances)?ms.instances.length:(Number(ms.imageCount||ms.count||0)||0);
            series.push(normalizeSeries({
    ...
    if(!p.name&&!p.id){
      const mp=raw?.study?.patient||raw?.patient;
      if(mp){
        p={name:String(mp.name||mp.PatientName||''),id:String(mp.id||mp.PatientID||''),birthDate:String(mp.birthDate||mp.PatientBirthDate||''),studyDate:String(raw?.study?.date||raw?.date||''),description:String(raw?.study?.description||raw?.description||''),accession:String(raw?.study?.accession||raw?.accession||'')};
      }
    }
    if(!p.name&&!p.id){
      try{
    ...
    const structSeries=Array.isArray(raw?.study?.series)?raw.study.series:(Array.isArray(raw?.series)?raw.series:null);
    return ctx.normalizeStudy({adapter:'DICOMWEB',studyUid:d.studyUid,patient:p,series:enriched,context:{rsBase:d.rsBase,seriesUrl,studyUid:d.studyUid,wadoTemplate:wado?.url||'',structureSeries:structSeries,completeKnown:true}});
  },
