...
export const DicomwebAdapter={
  ...
  match(summary){
    ...
    return Boolean(cur.includes('/viewer')&&/[?&]studies=/.test(cur)&&/[?&]session=/.test(cur));
  },
  async analyze(ctx){
    ...
    const raw=await ctx.fetchJson(seriesUrl,'application/dicom+json, application/json');
    const series=parseDicomwebSeries(raw);
    const enriched=[];
